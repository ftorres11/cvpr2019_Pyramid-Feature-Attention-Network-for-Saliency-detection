...
# ========================================================================
def tfLaplace(x):
    laplace = tf.constant([[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]],
              tf.float32)
...
def EdgeLoss(y_true, y_pred):
    ...
    edge_pos = 2.
    edge_loss = K.mean(\
            tf.nn.weighted_cross_entropy_with_logits(y_true_edge,
            y_pred, edge_pos), axis=-1)
    return edge_loss
...
def EdgeHoldLoss(y_true, y_pred):
    ...
    edge_loss = K.mean(\
            tf.nn.sigmoid_cross_entropy_with_logits(labels=y_true_edge,
            logits=y_pred_edge), axis=-1)
    saliency_pos = 1.12
    saliency_loss = K.mean(\
            tf.nn.weighted_cross_entropy_with_logits(y_true,y_pred,
            saliency_pos), axis=-1)
